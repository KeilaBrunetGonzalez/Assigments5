...
class EntityA:
    ...
    def handle_event_wait_for_call(self, e, arg=None):
        if e == self.OUTPUT:
            ...
            if TRACE > 0:
                print('EntityA: Timeout.')

        else:
            self.unknown_event(e)

    def handle_output_event(self):
    ...
    def handle_input_event(self, packet):
        if pkt_is_corrupt(packet) or packet.acknum != self.bit:
            return
    ...
    def unknown_event(self, e):
        print(f'EntityA: Unknown event {e}.')
...
class EntityB:
    ...
    def input(self, packet):
        if packet.seqnum != self.expecting_bit or pkt_is_corrupt(packet):
            self.send_ack(1 - self.expecting_bit, packet.payload)
        else:
            to_layer5(self, Msg(packet.payload))
            self.send_ack(self.expecting_bit, packet.payload)
            self.expecting_bit = 1 - self.expecting_bit

    def timer_interrupt(self):
        pass

    def send_ack(self, acknum, payload):
        p = Pkt(0, acknum, 0, payload)
        pkt_insert_checksum(p)
        to_layer3(self, p)
